...
def _plain_cert_pem(*, issuer_cn="me", subject_cn="me"):
    """An unmarked cert (no auto-generated Organization) + its key, PEM
    bytes. `issuer_cn != subject_cn` makes it look CA-issued."""
...
def test_reissues_a_broken_unmarked_self_signed_pair(tmp_path):
    # e.g. an auto-generated cert from before the marker existed, torn by
    # a restart loop. Self-signed + broken -> safe to replace.
    cert, key = tmp_path / "portal.crt", tmp_path / "portal.key"
    cert.write_bytes(_plain_cert_pem(issuer_cn="old", subject_cn="old")[0])
    key.write_bytes(_plain_cert_pem()[1])  # a different key -> mismatch
    ensure_self_signed_cert(cert, key, common_name="pve.example")
    parsed = x509.load_pem_x509_certificate(cert.read_bytes())
    assert parsed.subject.get_attributes_for_oid(NameOID.COMMON_NAME)[0].value == "pve.example"


def test_leaves_a_broken_CA_issued_pair_alone(tmp_path, caplog):
    cert, key = tmp_path / "portal.crt", tmp_path / "portal.key"
    cert.write_bytes(_plain_cert_pem(issuer_cn="Real CA", subject_cn="portal.example")[0])
    key.write_bytes(_plain_cert_pem()[1])  # wrong key
...
def test_leaves_an_unreadable_pair_alone(tmp_path, caplog):
    cert, key = tmp_path / "portal.crt", tmp_path / "portal.key"
